...
pnpm-workspace monorepo publishing four registry-agnostic `@karnameh/*` packages
(`styles`, `utils`, `ui`, `api`) for React apps. Stack: pnpm + tsdown (ESM+CJS+dts)
+ Changesets + TypeScript 5 + React 19 + Tailwind v4.
...
- **tsdown preserves `"use client"`/`"use server"` natively**, but only at the
  top of an entry's own source. Re-export-only barrels (`packages/utils/src/hooks/index.ts`,
  `packages/ui/src/index.ts`) carry `"use client";` explicitly — keep it.
