...
- **shadcn design system.** `@chatool/core` ships the shadcn token layer:
  standard CSS variables (`--background`, `--primary`, `--radius`, etc.) in `:root`/`.dark`,
  mapped to Tailwind utilities via `@theme inline` (`bg-primary`, `text-foreground`,
  `rounded-md`, `border-border`). Style components with token **utilities, never raw hex**;
  apps re-theme by overriding the standard CSS variables. Spec: `docs/conventions/shadcn.md`.
